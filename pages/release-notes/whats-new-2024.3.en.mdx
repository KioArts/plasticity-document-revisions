...
- **Section Analysis**
Section Analysis is a feature that allows users to slice through their 3D model to reveal its interior structure. This helps identify whether internal components are properly aligned or interfering with each other. It is especially useful for architectural, engineering, and design visualization. For more information, click [here](/common/section-analysis).
...
- Assets/Materials now have a right click context menu with "Select all with this material"
...
- Recent Files - Recent files/items can be dragged/dropped into viewport to import.
- Focus Point Orbitting - Shows a dot of focus point when orbiting.
- Side Bar - Pannels on right side will behave as menus.
- Side Bar - Toggle sidebar button now introduced.
- Viewport Cube - Repeatedly clicking on the cube will flip front/back and left/right, etc.
- Progress Indicator - Opening large files will show a simple progress indicator.
- Tooltips - Implemented tooltip images in certain tools on specific command options for better displaying function and concept of tool.
- P-menu - P-menu will now notify when there's a new version to download. You can disable in preferences.
- Thumbnails - Thumbnails can now be saved with files.

## 11. Command Bar

- Suggested commands (the "..." menu in the command bar) can be invoked with the keybinding shift+F.

## 12. Preferences 

- Grid Color - Set grid color in preferences.
- Start-up - Preference to disable loading previous session on start-up.

Section Analysis

- Section Plane - Change section plane by selecting faces in section analysis command.
- Section Plane - will show interference in red color.

Spacemouse (Applications)
- Spacemouse - Spacemouse buttons can be bound only via editing keymap.json file. Configuration file set-up can be nammed as "spacemouse0", "spacemouse1", for example.
