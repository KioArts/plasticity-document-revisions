...
Remove hidden surface CVs for more control. Only applicable when you've raised surface degree. Refer to [here](/solid/remove-nominal-surface).

### Toggle Points
Introduced Toggle Points command to show/hide points on faces, curves, and bodies. Refer to [here](/sketch/toggle-points).
...
Patch Command now optionally accepts an overbuilt sheet to trim into a hole and trim the main sheet to align with the end section of the base sheet. For more information, click [here](/solid/patch#trim-sheet-with-patch).
...
Introduced new edge snaps "closest, perpendicular, and tangent" allowing for more control and precision when working with edges. For more information on grid snapping features click [here](/plasticity-essentials/grid-and-snap).
...
Added new shaders for isophotes and draft angle analysis and new built-in matcaps for advanced surfacing workflows, including the option to insert custom matcaps. Refer to [here](/plasticity-essentials/view-mode).

### New Tangent Circle and Arc Hover Snaps
...
Added scaling options to the Pipe and Sweep commands. Refer to [Pipe](/solid/pipe) and [Sweep](/solid/sweep) for more details.
...
- December 18, 2023 [Version 1.4](/release-notes/whats-new-1-4)