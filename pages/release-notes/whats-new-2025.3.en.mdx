...
### New Publish to Plasticity Share Command
- **Upload and Share (.plasticity) Document Online**    
*Publish to Plasticity Share* is a new export command that lets you share a web link to your work. It uploads only the mesh data (no CAD geometry or source files), so the shared file can be only previewed and cannot be downloaded, keeping cloud storage usage minimal. This feature is ideal for quickly showing visualizations without exposing the original project files.  
...
## Support 
- **Join Discord**    
Those who have a license can join the official Discord server for further support. The community is active and growing every day!
