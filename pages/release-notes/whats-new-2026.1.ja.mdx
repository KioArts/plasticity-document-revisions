...
PolySplines は、メッシュオブジェクト（.OBJ、.STL など）をPlasticity 内の編集可能なNURBSサーフェスに変換するサードパーティの新コマンドです。G2連続性を持つクリーンな単スパン・複数スパンのサーフェスを生成します。これはStudioのみの機能であり、Blender Bridgeアドオンと組み合わせて使用できます。
