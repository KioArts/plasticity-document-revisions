...
2. **Cinema 4D内:**
    ...
    4. **Subdivision Surface** オブジェクトを作成し、**Outbox** ヌルの中に配置します。コントロールケージのジオメトリ(ポリゴンオブジェクト、立方体や球体のようなプリミティブ、またはシンメトリー、接続、ブール、アレイのような任意のジェネレータースタック)を、そのSubdivision Surfaceの子として配置します。Subdivision Surfaceのエディター細分割レベルは **最低 1** に設定する必要があります。
        - コントロールケージは自動的に解決されます。プリミティブ、ジェネレーター、さらには入れ子になったジェネレータースタックもCinema 4Dのキャッシュシステムを通じて評価されるため、事前にエディタブルにする必要なく、ライブのプロシージャルセットアップをアップロードできます。
