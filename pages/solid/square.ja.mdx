...
3. コマンドダイアログでオプションを設定します。  
   ...
   - **Scale:**  
       - 境界エッジ上に表示される赤い線のサイズを調整します。  
       - これは **「エッジ偏差プロット (Edge Deviation Plot)」** を表し、生成されたサーフェスの境界と入力サーフェスの距離を示します。  
       - 赤色は 0.01mm (1e-5m) 以上の偏差を示し、入力ジオメトリと一致しない領域を強調します。  
...
**Square でのロフト**  
代替として、Square コマンドはロフト手法として使用することができ、2 つの接触していないエッジを選択することで適用できます。この方法では、度数やスパン値、境界条件（必要に応じて）、平坦度、ウェイト影響などのパラメータを指定することができ、より柔軟な制御が可能です。一方、標準的なロフトを使用すると、制御オプションが限られた固定されたサーフェスが生成されます。  
</Callout>
<VideoPlayer fileName="square-1"/>

<Callout type="info" emoji="ℹ️">
**Square でのスイープ**  
Sweep コマンドと同様に、Square コマンドは同等の結果を生成できますが、より柔軟で詳細な制御が可能です。この結果は、2 つまたは 3 つの接触しているエッジを選択することで達成できます。  
</Callout>
<VideoPlayer fileName="square-2"/>

<Callout type="info" emoji="ℹ️">
**Square でのパッチング**  
Square コマンドは、Patch コマンドと同様に、四辺の境界エッジを持つ穴をサーフェスで埋めることができます。  
</Callout>
<VideoPlayer fileName="square-3"/>

<Callout type="info" emoji="💡">
**ヒント**  
XNurbs と同様に、コマンドを事前に実行し、境界エッジ、カーブ、サーフェスを選択できます。これにより、ワークフローを効率的に進めることができ、サーフェス生成のライブプレビューを提供します。  
</Callout>
<VideoPlayer fileName="square-4"/>

<Callout type="info" emoji="💡">
**ヒント**  
Square コマンドは、顔とその隣接する面上のトリムされたサーフェスを選択的に再調整し、サーフェスの自然なフローに一致するようにスプラインの向きを整列させます。この機能は、スプラインの向きを調整せずにサーフェスを再構築することに焦点を当てた **Rebuild** コマンドとは大きく異なります。  
</Callout>
